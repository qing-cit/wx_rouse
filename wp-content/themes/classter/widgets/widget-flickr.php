	...
	function widget( $args, $instance ) {
		...
		<div class="flickr_images">
			...
			if ($flickr_count <= 10) {
				...
				?>
				<script type="text/javascript" src="<?php echo themerex_get_protocol(); ?>://www.flickr.com/badge_code_v2.gne?count=<?php echo esc_attr($flickr_count); ?>&amp;display=random&amp;flickr_display=random&amp;size=<?php echo esc_attr($size); ?>&amp;layout=x&amp;source=user&amp;user=<?php echo esc_attr($flickr_username); ?>"></script>
				<?php
			} else {
				// New method > 10 images
				$size = $THEMEREX_CURRENT_SIDEBAR == 'top' ? 'mid' : 'square';
				?>
				<script type="text/javascript" src="<?php echo themerex_get_protocol(); ?>://www.flickr.com/badge_code.gne?count=<?php echo esc_attr($flickr_count); ?>&amp;display=random&amp;flickr_display=random&amp;size=<?php echo esc_attr($size); ?>&amp;layout=x&amp;source=user&amp;nsid=<?php echo esc_attr($flickr_username); ?>&amp;raw=1"></script>
				<?php
			}
			?>
		</div>
