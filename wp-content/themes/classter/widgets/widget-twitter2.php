...
class Twitter2_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		foreach ($data as $tweet) {
			...
			$output .= '<li class="theme_text' . ($cnt==$twitter_count-1 ? ' last' : '') . '"><a href="'.themerex_get_protocol().'://twitter.com/' . $twitter_username . '" class="username" target="_blank">@' . $tweet['user']['screen_name'] . '</a> ' . twitter_prepare_text($tweet) . '</li>';
