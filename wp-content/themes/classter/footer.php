...
global $logo_footer, $logo_text;
			...
			<div class="footerContentWrap">
				...
				if ( get_custom_option('googlemap_show')=='yes' ) {
					...
					<?php }
					$map_title = get_custom_option('googlemap_title');
					$map_description = get_custom_option('googlemap_description');
					...
					if (!empty($map_address) || !empty($map_latlng)) {
						echo do_shortcode('[trx_googlemap'
							. (!empty($map_title) ? ' title="'.addslashes($map_title).'"' : '')
							. (!empty($map_description) ? ' description="'.addslashes($map_description).'"' : '')
							. (!empty($map_address) ? ' address="'.addslashes($map_address).'"' : '')
							. (!empty($map_latlng) ? ' latlng="'.addslashes($map_latlng).'"' : '')
							. (!empty($map_style) ? ' style="'.addslashes($map_style).'"' : '')
							. (!empty($map_zoom) ? ' zoom="'.addslashes($map_zoom).'"' : '')
							. (!empty($map_height) ? ' height="'.addslashes($map_height).'"' : '')
							. ']');
					}
				}
				?>
