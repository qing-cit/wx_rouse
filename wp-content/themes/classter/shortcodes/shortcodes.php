...
function sc_banner($atts, $content=null){	
	...
	$output = empty($src) ? '' : ('<a href="' . (empty($link) ? '#' : esc_url($link)) . '"' . ($id ? ' id="' . $id . '"' : '')
...
function sc_button($atts, $content=null){	
	...
	$output = '<'.(sc_param_is_on($block) ? 'div' : 'span').' class="sc_button sc_button_style_' . $style . ' sc_button_size_' . $size . ($align && $align!='none' ? ' align'.$align : '') . (!empty($class) ? ' '.$class : '')
...
function sc_columns($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'. (sc_param_is_off($indentation) ? 'columnsFloat ' : '') .''. (sc_param_is_on($autoheight) ? 'autoHeight ' : '') .'columnsWrap sc_columns sc_columns_count_' . $count . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'>' . do_shortcode($content).'</div>';
...
function sc_contact_form($atts, $content = null) {
	...
	if($style!='order') {
		...
		$output = '<div ' . ($id ? ' id="' . $id . '"' : '') . 'class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_contact_form_' . ($content ? 'custom' : 'contact') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
...
function sc_countdown($atts, $content = null) {
	...
	if (date('Y-m-d H:i:s') < $date.' '.($time ? $time : '00:00:00')) {
		...
		$s = getStyleString($top, $right, $bottom, $left, $width, $height);
		$class .= getStyleMargin($top, $right, $bottom, $left);
		$output = '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_countdown_wrapper' . ($align && $align!='none' ? ' align' . $align : '') . (!empty($class) ? ' '.$class : '') .'"'.($s ? ' style="'.$s.'"' : '').'><div class="sc_countdown" data-date="'.$date.'" data-time="'.$time.'"></div></div>';
	}
...
function sc_emailer($atts, $content = null) {
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_emailer inputSubmitAnimation' . ($align && $align!='none' ? ' sc_align' . $align : '') . (sc_param_is_on($open) ? ' sFocus opened' : ' radCircle') . (!empty($class) ? ' '.$class : '') . '"' . ($s ? ' style="'.$s.'"' : '') . '>'
...
function sc_image($atts, $content=null){	
	...
	if(sc_param_is_on($link)){
		$output = empty($src) || empty($url_color) ? '' : ('<a href="'.esc_url(trim($href)).'"' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image sc_image_hover' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
			. '<img src="' . $src . '" alt="" />'
		...
		$output = empty($src) ? '' : ('<figure data-image="' . $hoverSrc . '" data-title="' . trim($title) . '"' . ($id ? ' id="' . $id . '"' : '') . ' class="thumb hoverIncrease sc_image ' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
...
function sc_infobox($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_infobox sc_infobox_style_' . $style . (sc_param_is_on($closeable) ? ' sc_infobox_closeable' : '') . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_line($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_line' . ($style != '' ? ' sc_line_style_' . $style : '') . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'></div>';
...
function sc_list($atts, $content=null){	
	...
	$output = '<' . ($style=='ol' ? 'ol' : 'ul') . ($id ? ' id="' . $id . '"' : '') . ' class="sc_list sc_list_style_' . $style . (!empty($class) ? ' '.$class : '') . '"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			. do_shortcode($content) 
			. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
...
function sc_price_table($atts, $content=null){	
	...
	$THEMEREX_sc_price_table_counter = 0;
	$count = max(1, $count);
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'.($style ? 'sc_pricing_'.$style.' ' : '').'sc_pricing_table columns_' . $count . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.$class : '') . '"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			. do_shortcode($content)
		. '</div>';
...
function sc_price_item($atts, $content=null) {
	...
	$output = '<div class="'.(sc_param_is_on($active) ? 'active ' : '').'sc_pricing_columns sc_pricing_column_'.$THEMEREX_sc_price_table_counter. (!empty($class) ? ' '.$class : '').'"><ul'.(sc_param_is_on($animation) ? ' class="columnsAnimate"' : '') . ($id ? ' id="' . $id . '"' : '') . '>'
		. do_shortcode($content) 
		. '</ul></div>';
...
function sc_price_data($atts, $content=null) {
	...
	if ($type=='price' && $money!='') {
		...
		$content = do_shortcode($content);
	$output = '<li' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_pricing_data' . ($type!='' ? ' sc_pricing_'.$type : '') . (!empty($class) ? ' '.$class : '') . '">' . $content . '</li>';
...
function sc_quote($atts, $content=null){	
	...
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . $content . '</p>';
	$output = '<blockquote' . ($id ? ' id="' . $id . '"' : '') . $cite_param . ' class="sc_quote'. (!empty($class) ? ' '.$class : '').' sc_quote_style_' . $style.'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
		. $content
...
function sc_table($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_table sc_table_style_' . max(1, min(4, $style)) . ' sc_table_size_' . $size . (!empty($align) ? ' sc_table_align_'.$align : '') . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '') .'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_testimonials($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_testimonials sc_testimonials_style'.(sc_param_is_off($controls) ? '' : ' sc_testimonials_controls_'.$controls) . (!empty($class) ? ' '.$class : '') .'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
...
function sc_text_slider($atts, $content=null){
	...
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_text_slider' . (!empty($class) ? ' '.$class : '') .'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
	...
	. '</div>';
	return apply_filters('themerex_shortcode_output', $output, 'trx_text_slider', $atts, $content);
}
...
function sc_text_slider_item($atts, $content=null){
	...
	$output = '<li' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_text_slider_item slides swiper-slide'. (!empty($class) ? ' '.$class : '').'" style="'.(!empty($THEMEREX_sc_text_slider_width) ? 'width:' . $THEMEREX_sc_text_slider_width . ';' : '').(!empty($THEMEREX_sc_text_slider_height) ? 'height:' . $THEMEREX_sc_text_slider_height . ';' : '').'">'
	. '<div class="sc_text_slider_item_content">'
	. '<div class="sc_text_slider_item_text">'.do_shortcode($content).'</div>'
	. '</div>'
	. '</li>';
	return apply_filters('themerex_shortcode_output', $output, 'trx_text_slider_item', $atts, $content);
}
...
function sc_sidebar($atts, $content=null){
	...
	$output = '<div class="widget_area sidebarStyleLight sc_sidebar_selector'.(!empty($class) ? $class : '').'"'.(!empty($s) ? ' style="'.$s.'"' : '').'>'.$sidebar_content.'</div>';
