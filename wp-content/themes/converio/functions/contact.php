...
function converio_send_mail($name, $subject, $email, $message) {
	...
	if($response['msgStatus'] == "ok") {
		echo '<p class="msg success"><a class="hide" href="#">' . esc_attr__('hide this', 'multipurpose') . '</a>' . esc_attr($response['message']) . '</p>';
	} else {
		if($response['errors']) {
			...
			echo '<p class="msg error"><a class="hide" href="#">' . esc_attr__('hide this', 'multipurpose') . '</a>' . esc_attr($response['message']) . '</p>';
		}
	}
	return $response;
}
